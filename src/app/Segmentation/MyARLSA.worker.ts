...
export const GetSegments = (d: any) => {
  ...
  class MyARLSA {
    ...
    public run(
      ...
      const initblobs1: blobObject[] = blobCounter.GetObjectsWithoutArray(
        this.myimage
      );
      //console.log(blobCounter.getObjectsCount())

      const WidthArray: number[] = initblobs1
        .map((v) => v.width)
        .filter((v) => v <= 20);
      ...
      if (WidthArray.length == 0) {
        ...
        AverageWidth =
          WidthArray.reduce((sum, v) => sum + v) / WidthArray.length;
      }
      ...
      this.myimage = myFilter.FilterOut(
        new ImageData(removelines.data, removelines.width, removelines.height),
        PunctuationMarks
      );
      ...
      const res: ImageData = this.DynHorRLA(
        this.ARLSA_Th,
        this.ARLSA_c,
        this.ARLSA_a
      );
      ...
      for (const b in Fblobs) {
        ...
        for (let y = 0; y < Fblobs[b].height; y++) {
          ...
          for (let x = 0; x < Fblobs[b].width; x++) {
            newarray[y][x] =
              timg.data[4 * (Fblobs[b].x + x) + (Fblobs[b].y + y) * stride] ==
                0 && Fblobs[b].Array[y][x]
                ? true
                : false;
          }
        }
      ...
      for (const b in blobs) {
        if (blobs[b].width >= minwidth) {
          ...
          for (
            let x = blobs[b].x;
            x <= blobs[b].Right;
            x++ //for (let x = blobs[b].x; x < blobs[b].Right; x++)
          ) {
            for (
              let y = blobs[b].y;
              y <= blobs[b].Bottom;
              y++ //for (let y = blobs[b].y; y < blobs[b].Bottom; y++)
            ) {
              image.data[4 * x + y * stride] = arlaimg.data[4 * x + y * stride];
              image.data[4 * x + y * stride + 1] =
                arlaimg.data[4 * x + y * stride + 1];
              image.data[4 * x + y * stride + 2] =
                arlaimg.data[4 * x + y * stride + 2];
            }
          }
        }
      }
    ...
    private xRLSA(x_c: number, img: ImageData, b: blobObject): void {
      ...
      for (
        let y = b.y;
        y <= b.Bottom;
        y++ //for (let y = b.y; y < b.Bottom; y++)
      ) {
        for (
          let x = b.x;
          x <= b.Right;
          x++ //for (let x = b.x; x < b.Right; x++)
        ) {
    ...
    private HistogramSpaces(proj1: number[]): number[] {
      ...
      if (min > 0) {
        for (let x = 0; x < proj.length; x++) proj[x] = proj[x] - min;
      }
    ...
    private PreFiltering(blobs: blobObject[]): blobObject[] {
      ...
      for (let i = 0; i < blobs.length; i++) {
        if (
          blobs[i].height <= this.myimage.height / 4 &&
          blobs[i].height > 5 &&
          blobs[i].width > 5
        ) {
    ...
    private DynHorRLA(
      HeightRatio_th: number,
      overlapping_c: number,
      blockdistance_a: number,
      CustomImage: ImageData = null
    ): ImageData {
      ...
      const myBMP: ImageData =
        CustomImage == null ? copy(this.myimage) : CustomImage;
      ...
      for (I = 1; I < bH - 1; I++) {
        for (J = 1; J < bW - 2; J++) {
          if (!PassArr[I][J]) {
            if (
              this.CCsLabelsArray[I][J] > 0 &&
              this.CCsLabelsArray[I][J + 1] <= 0
            ) {
              PassArr[I][J + 1] = true;
              ...
              if (this.CCsLabelsArray[I][K] == id) {
                for (L = Store; L < K; L++) Temp[I][L] = true;
              } else {
                ...
                Hoverlapping = Math.abs(
                  Math.max(CC1.y, CC2.y) -
                    Math.min(CC1.Bottom + 1, CC2.Bottom + 1)
                );
                if (
                  HRatio <= HeightRatio_th &&
                  ...
                  for (L = Store; L < K; L++)
                    if (this.CheckNeighborhood(I, L, id, id2, bW) == false) {
                      ...
                      for (M = Store; M <= L; M++) Temp[I][M] = false;
                    }
                }
              }
            }
    ...
    private CheckNeighborhood(
      ...
      for (let R = I - 1; R <= I + 1; R++) {
        for (let C = L - 1; C <= L + 1; C++) {
          if (
            this.CCsLabelsArray[R][C] > 0 &&
            this.CCsLabelsArray[R][C] != id &&
            this.CCsLabelsArray[R][C] != id2
          ) {
            Result = true;
          }
        }
        ...
        for (let yy = 0; yy < Rejblobs[b].height; yy++) {
          for (let xx = 0; xx < Rejblobs[b].width; xx++) {
            if (Rejblobs[b].Array[yy][xx]) {
              myimage.data[
                4 * (Rejblobs[b].x + xx) + (Rejblobs[b].y + yy) * stride
              ] = 255;
              myimage.data[
                4 * (Rejblobs[b].x + xx) + (Rejblobs[b].y + yy) * stride + 1
              ] = 255;
              myimage.data[
                4 * (Rejblobs[b].x + xx) + (Rejblobs[b].y + yy) * stride + 2
              ] = 255;
              myimage.data[
                4 * (Rejblobs[b].x + xx) + (Rejblobs[b].y + yy) * stride + 3
              ] = 255;
            }
          }
        }
    ...
    private ExtractBlobs(
      FinalBlobs: blobObject[],
      width: number,
      height: number
    ): ImageData {
        ...
        for (let yy = 0; yy < FinalBlobs[b].height; yy++) {
          for (let xx = 0; xx < FinalBlobs[b].width; xx++) {
            if (FinalBlobs[b].Array[yy][xx]) {
              myimage.data[
                4 * (FinalBlobs[b].x + xx) + (FinalBlobs[b].y + yy) * stride
              ] = 0;
              myimage.data[
                4 * (FinalBlobs[b].x + xx) + (FinalBlobs[b].y + yy) * stride + 1
              ] = 0;
              myimage.data[
                4 * (FinalBlobs[b].x + xx) + (FinalBlobs[b].y + yy) * stride + 2
              ] = 0;
              myimage.data[
                4 * (FinalBlobs[b].x + xx) + (FinalBlobs[b].y + yy) * stride + 3
              ] = 255;
            }
          }
        }
  ...
  class Filtering {
    ...
    private RemovePanctuation(myImage: ImageData): void {
      ...
      for (const b in blobs) {
        ...
        for (let y = 0; y < blobs[b].height; y++) {
          for (let x = 0; x < blobs[b].width; x++) {
            if (
              myImage.data[4 * (blobs[b].x + x) + (blobs[b].y + y) * stride] ==
              0
            ) {
              p1++;
            }
            if (
              imgi2.data[4 * (blobs[b].x + x) + (blobs[b].y + y) * stride] == 0
            ) {
              p2++;
            }
          }
        }
        if (p2 / p1 - this.ARLSA.Pr <= 0)
          //((p2 / p1).CompareTo(ARLSA.Pr) <= 0)
    ...
    public Filter(): void {
      ...
      for (const b in this.FirstPassBlobs) {
        if (
          //this.FirstPassBlobs[b].height <= AHE / 4 ||
    ...
    private InitFilter(): void {
      for (const b in this.InitBlobs) {
        ...
        else this.RejBlobs.push(this.InitBlobs[b]);
      }
    }

    public FinalFiltering(
      InputBlobs: blobObject[],
      PassBlobs: blobObject[],
      RejBlobs: blobObject[]
    ): void {
      ...
      for (const b in InputBlobs) {
        ...
        else PassBlobs.push(InputBlobs[b]);
      }
    }
        ...
        for (let yy = 0; yy < RemBlobs[b].height; yy++) {
          for (let xx = 0; xx < RemBlobs[b].width; xx++) {
            if (RemBlobs[b].Array[yy][xx]) {
              myImage.data[
                4 * (RemBlobs[b].x + xx) + (RemBlobs[b].y + yy) * stride
              ] = 255;
              myImage.data[
                4 * (RemBlobs[b].x + xx) + (RemBlobs[b].y + yy) * stride + 1
              ] = 255;
              myImage.data[
                4 * (RemBlobs[b].x + xx) + (RemBlobs[b].y + yy) * stride + 2
              ] = 255;
              myImage.data[
                4 * (RemBlobs[b].x + xx) + (RemBlobs[b].y + yy) * stride + 3
              ] = 255;
            }
          }
        }
      ...
      for (const b in this.FirstPassBlobs) {
        ...
        for (let y = 0; y < this.FirstPassBlobs[b].height; y++) {
          for (let x = 0; x < this.FirstPassBlobs[b].width; x++) {
            if (this.FirstPassBlobs[b].Array[y][x]) {
              blacks++;
            }
          }
        }

        this.FirstPassBlobs[b].Density =
          blacks /
          (this.FirstPassBlobs[b].width * this.FirstPassBlobs[b].height);
        this.FirstPassBlobs[b].Elongation =
          Math.min(
            this.FirstPassBlobs[b].width,
            this.FirstPassBlobs[b].height
          ) /
          Math.max(this.FirstPassBlobs[b].width, this.FirstPassBlobs[b].height);
      }
    }
  ...
  const objects: blobObject[] = arlsa.run(
    d.imageData,
    d.RemovePunctuationMarks,
    d.Xrlsa
  );
