    ...
    for (const b in blobs) {
      ...
      for (let y = 0; y < blobs[b].height; y++) {
        for (let x = 0; x < blobs[b].width; x++) {
          if (
            myImage.data[4 * (blobs[b].x + x) + (blobs[b].y + y) * stride] == 0
          ) {
            p1++;
          }
          if (
  ...
  public CalcDensityAndElong(): void {
    ...
    for (const b in this.FirstPassBlobs) {
      ...
      this.FirstPassBlobs[b].Density =
        blacks / (this.FirstPassBlobs[b].width * this.FirstPassBlobs[b].height);
      this.FirstPassBlobs[b].Elongation =
        Math.min(this.FirstPassBlobs[b].width, this.FirstPassBlobs[b].height) /
