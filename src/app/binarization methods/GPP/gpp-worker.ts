...
function GPP(d: MessageEvent){
   ...
   const gppMethod = new GppBinarization(data);
...
class GppBinarization{
  ...
  binarize( width, height, dw, k, R, q, p1, p2, upsampling, dw1){
   ...
   greyscale() {
       ...
       for(let i = 0; i < this.data.length; i += 4) {
             const brightness = RED_INTENCITY_COEF * this.data[i] + GREEN_INTENCITY_COEF * this.data[i + 1] + BLUE_INTENCITY_COEF * this.data[i + 2];
   ...
   gpp( width, height, _dw, _k, _R, _q, _p1, _p2, _upsampling, _dw1){
       ...
       for (let i = 0; i < height; i++)
       {
         ...
         {
            IMAGE[i][k] = this.data[4 * k + i * 4 * width];
            IMAGE11[i][k] = this.data[4 * k + i * 4 * width];
         }
       }
        ...
        for (let i = 0; i < height; i++)
       {
         for (let k = 0; k < width; k++)
         {
           ...
           this.data[4 * k + i * 4 * width] = IMAGE2[i][k];
           this.data[4 * k + i * 4 * width + 1] = IMAGE2[i][k];
           this.data[4 * k + i * 4 * width + 2] = IMAGE2[i][k];
