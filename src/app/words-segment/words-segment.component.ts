...
export class WordsSegmentComponent {
  ...
  enableBoundaryLimit() {
    this.canvas.observe('object:moving', (e: any) => {
      ...
      if (obj && obj.type == 'line') {
        ...
        this.CalcTextInputCords(
          obj.oCoords.tl.x,
          obj.oCoords.tl.y,
          obj.oCoords.tr.x - obj.oCoords.tl.x + 1
        ); //update TextInput div Cords on object moving
      }
  ...
  enableZoom() {
    this.canvas.on('mouse:wheel', (opt: any) => {
      ...
      if (activeObject && activeObject.type == 'rect') {
        this.CalcTextInputCords(
          activeObject.oCoords.tl.x,
          activeObject.oCoords.tl.y,
          activeObject.oCoords.tr.x - activeObject.oCoords.tl.x + 1
        );
      }
    });
  ...
  mergeObjectsCoords(
    ...
    const h =
      Math.round(activeObject.height) - yTop * ratioY - yBottom * ratioY;
  ...
  CalcNextInputCoords() {
    ...
    this.CalcTextInputCords(
      nextOb.oCoords.tl.x,
      nextOb.oCoords.tl.y,
      nextOb.oCoords.tr.x - nextOb.oCoords.tl.x + 1
    );
    setTimeout(() => {
      this.inputField.nativeElement.focus();
    }, 0);
  ...
  Segmentation(imageData: ImageData) {
    this.workerService
      .run(GetSegments, {
        imageData: imageData,
        ARLSA_a: this.ARLSA_a,
        ARLSA_c: this.ARLSA_c,
        ARLSA_Th: this.ARLSA_Th,
        RemovePunctuationMarks: this.RemovePunctuationMarks,
        Xrlsa: this.Xrlsa,
      })
      .then((objects: any) => {
        //console.log(objects)
  ...
  drawGTrects() {
    ...
    if (this.GTdraw) {
      ...
      const objects = this.canvas
        .getObjects()
        .filter((obj) => obj.rectType == 'gtRect');
      for (const i in objects) {
        this.canvas.remove(objects[i]);
      }
    }
