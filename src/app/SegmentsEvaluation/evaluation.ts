...
export default class evaluation {
  ...
  public run(
    GroundTruthRects: Rectangles[],
    MyArlsaRects: Rectangles[],
    Tinter = 0.5
  ) {
    let overlaps = 0;
    ...
    for (const i in GroundTruthRects) {
      ...
      const br1 = {
        x: GroundTruthRects[i].x + GroundTruthRects[i].width - 1,
        y: GroundTruthRects[i].y + GroundTruthRects[i].height - 1,
      };

      for (const j in MyArlsaRects) {
        const tl2 = { x: MyArlsaRects[j].x, y: MyArlsaRects[j].y };
        const br2 = {
          x: MyArlsaRects[j].x + MyArlsaRects[j].width - 1,
          y: MyArlsaRects[j].y + MyArlsaRects[j].height - 1,
        };
